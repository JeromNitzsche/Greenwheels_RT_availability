import httpx
import pandas as pd  # (nog aanwezig; kan weg als je het nergens gebruikt)
from datetime import datetime, timedelta
from dateutil import tz
import json
import os
from flask import make_response  # (nog aanwezig; kan weg als je het nergens gebruikt)
import logging
...
# Endpoint: default naar de nieuwe endpoint, env var mag overriden
GRAPHQL_URL = os.getenv("GRAPHQL_URL", "https://graphql.greenwheels.com")

# Guardrail: voorkom per ongeluk gebruik van de (mogelijk) te blokkeren endpoint
if "www.greenwheels.com/api/graphql" in GRAPHQL_URL:
    raise ValueError(
        "Misconfiguratie: gebruik NIET https://www.greenwheels.com/api/graphql. "
        "Gebruik https://graphql.greenwheels.com."
    )

# Apollo identificatie volgens Greenwheels instructies
CLIENT_NAME = "greenest"
CLIENT_VERSION = os.getenv("GREENWHEELS_CLIENT_VERSION", "v1.0.0")
...
def clean_license(lic: str) -> str:
    return (lic or "").replace("-", "")

def update_availability(request):
    logger.info("Starting update_availability function.")
    logger.info(f"Using GRAPHQL_URL={GRAPHQL_URL}")
    logger.info(
        f"Apollo headers: apollographql-client-name={CLIENT_NAME}, "
        f"apollographql-client-version={CLIENT_VERSION}"
    )
    ...
    # Reuse httpx client voor performance/stabiliteit
    with httpx.Client(timeout=30.0) as client:
        for start, end in blocks:
            ...
            try:
                ...
                if resp.status_code != 200:
                    logger.error(f"Non-200 response: {resp.status_code} - {resp.text}")
            ...
            for loc in locations:
                ...
                lng = (loc.get("geoPoint") or {}).get("lng")

                for car in loc.get("cars", []):
                    car_id = car["id"]
                    available = (car.get("availability") or {}).get("available", True)
        ...
        for start, end in missing_blocks:
            ...
            try:
                ...
                if resp.status_code != 200:
                    logger.error(f"Non-200 response on retry: {resp.status_code} - {resp.text}")
                ...
                for loc in locations:
                    for car in loc.get("cars", []):
                        if not (car.get("availability") or {}).get("available", True):
                            conflict_dict.setdefault(car["id"], []).append((start, end))
            except Exception as e:
                logger.error(f"Herhaalverzoek voor blok {start}–{end} mislukt: {e}")

    def merge_blocks(blocks_list):
        if not blocks_list:
            return []
        blocks_list.sort()
        merged = [blocks_list[0]]
        for s, e in blocks_list[1:]:
    ...
    def format_blocks(blocks_list):
        return ", ".join(f"{s.strftime('%H:%M')}–{e.strftime('%H:%M')}" for s, e in blocks_list)
    ...
    # Als je dit als HTTP Cloud Function gebruikt, kun je hier desgewenst een response teruggeven:
